...
const SidePanel = () => {
  ...
  // Plan awaiting the user's approval (PLAN_WAIT); null = nothing pending
  const [pendingPlan, setPendingPlan] = useState<string | null>(null);
  // ask_user question awaiting the user's typed answer (ASK_USER); null = none
  ...
  // A file dragged anywhere in the panel must not make Chrome open it in place of the
  // panel; only the chat input takes the drop (its own handler runs before this one).
  useEffect(() => {
  ...
  // Zoom the panel with Ctrl/Cmd +/-/0 (Chrome's page zoom doesn't reach a side
  // panel, so we scale the root ourselves and remember it). Shift is allowed so
  // Ctrl+Shift+= (the '+' key) works too.
  useEffect(() => {
    const KEY = 'koni-panel-zoom';
    const root = document.documentElement;
    // style.zoom also scales 100vh, so the panel's full-height root divides by --koni-zoom
    // to stay exactly one viewport tall (otherwise zooming in pushes the input off-screen).
    const apply = (z: number) => {
      root.style.zoom = String(z);
      root.style.setProperty('--koni-zoom', String(z));
    };
    let zoom = parseFloat(localStorage.getItem(KEY) || '1') || 1;
    if (zoom !== 1) apply(zoom);
    const onKey = (e: KeyboardEvent) => {
      if (!(e.ctrlKey || e.metaKey)) return;
      if (e.key === '+' || e.key === '=') zoom = Math.min(1.8, zoom + 0.1);
      else if (e.key === '-' || e.key === '_') zoom = Math.max(0.6, zoom - 0.1);
      else if (e.key === '0') zoom = 1;
      else return;
      e.preventDefault();
      zoom = Math.round(zoom * 10) / 10; // keep 0.1 steps exact, no float drift
      apply(zoom);
  ...
  const handleTaskState = useCallback(
    (event: AgentEvent) => {
      ...
      switch (actor) {
        case Actors.SYSTEM:
          switch (state) {
            case ExecutionState.TASK_START:
              // Reset historical session flag when a new task starts. From
              // here on, anything the user types belongs to THIS session —
              // mid-run sends go out as follow_up_task (queued at a step
              // boundary by the background). Also clears a stale ask card.
              setIsHistoricalSession(false);
              setIsFollowUpMode(!isReplayingRef.current);
              setPendingPlan(null);
            ...
            case ExecutionState.TASK_CANCEL:
              // Stay in the same session after a cancel: the next message goes
              // out as a follow_up_task carrying this sessionId, so the
              // background can revive the saved record (Wave 2) instead of
              // starting a blank task. Cancelled replays keep starting fresh.
              setIsFollowUpMode(!isReplayingRef.current);
              setInputEnabled(true);
              setShowStopButton(false);
              setIsReplaying(false);
            ...
            case ExecutionState.TASK_RESUME:
              break;
            case ExecutionState.ASK_USER:
              // ask_user (Wave 3): the run ended awaiting the user's typed
              // answer — show the question card and keep the input open. The
              // answer goes out as a normal follow_up_task and continues the
              // session (live executor or revival).
              setPendingQuestion(content || '');
              setIsFollowUpMode(true);
              setInputEnabled(true);
              setShowStopButton(false);
              break;
            case ExecutionState.USER_MESSAGE:
              // Mid-run user chat: confirmation that a message typed while the
              // agent was running landed in its context at a step boundary.
              skip = false;
              break;
            default:
        ...
        case Actors.PLANNER:
          switch (state) {
            case ExecutionState.PLAN_WAIT:
              // Executor is paused waiting for approval — show the card, not a message
              setPendingPlan(content || '');
              break;
  ...
  // `docs` carries store-path office documents (structured parses) so a big
  // document is never inlined into the task text; it rides the same port
  // message and the background keeps it on the AgentContext (doc store).
  const handleSendMessage = async (text: string, displayText?: string, docs?: AttachedDocPayload[]) => {
    ...
    try {
      ...
      // Mid-run user chat (Wave 3): the input stays ENABLED while the agent
      // runs — messages sent mid-run ride the same follow_up_task port
      // message and are queued by the background for step-boundary injection.
      // (Replay still disables the input; a replaying executor has no queue.)
      setShowStopButton(true);
      // Typing while the ask_user card is up IS the answer — clear the card.
      setPendingQuestion(null);

      // Historical sessions keep their sessionId and go out as follow-ups so
      // the background can revive the saved session record (or start blank if
      // none exists). Otherwise, create a new chat session when not in
      // follow-up mode.
      if (!isFollowUpMode && !isHistoricalSession) {
      ...
      if (isFollowUpMode || isHistoricalSession) {
        ...
        // From now on every send in this session is a follow-up (mid-run
        // messages included) — don't wait for TASK_START to flip the flag.
        setIsFollowUpMode(true);
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : String(err);
  ...
  // Plan approval: resume the paused executor / cancel the task
  ...
  return (
    <div>
      <div
        className={`flex h-[calc(100vh/var(--koni-zoom,1))] flex-col ${isDarkMode ? 'bg-slate-900' : 'bg-koni-neutral-50'} overflow-hidden border ${isDarkMode ? 'border-sky-800' : 'border-koni-neutral-200'}`}>
        <header
          className={`header relative ${isDarkMode ? '' : 'bg-white border-b border-koni-neutral-200'} px-3 py-2.5`}>
          <div className="header-logo">
            {showHistory ? (
              <button
                type="button"
                onClick={() => handleBackToChat(false)}
                className={`${isDarkMode ? 'text-sky-400 hover:text-sky-300' : 'text-koni-neutral-500 hover:text-koni-primary-600'} cursor-pointer`}
                aria-label={t('nav_back_a11y')}>
                {t('nav_back')}
              </button>
            ) : hasConfiguredModels === true ? (
              /* The app name already lives in the panel title bar — Claude-style,
                 this spot shows the active model instead. */
              <ModelSelector isDarkMode={isDarkMode} />
            ) : (
              <div className="flex items-center gap-2">
                <img src="/icon-128.png" alt="Extension Logo" className="size-7" />
                <span
                  className={`text-sm font-semibold tracking-tight ${isDarkMode ? 'text-gray-100' : 'text-koni-navy-900'}`}>
                  KONI
                </span>
              </div>
            )}
            ...
            {!showHistory && (
              <>
                <button
                  ...
                  className={`header-icon ${isDarkMode ? 'text-sky-400 hover:text-sky-300' : 'text-koni-neutral-500 hover:text-koni-primary-600'} cursor-pointer`}
        ...
        {showHistory ? (
          ...
          <>
            ...
            {hasConfiguredModels === null && (
              <div
                className={`flex flex-1 items-center justify-center p-8 ${isDarkMode ? 'text-sky-300' : 'text-koni-neutral-700'}`}>
                <div className="text-center">
                  <div className="mx-auto mb-4 size-8 animate-spin rounded-full border-2 border-koni-primary-600 border-t-transparent"></div>
            ...
            {hasConfiguredModels === false && (
              <div
                ...
                <div className="max-w-md space-y-4 text-center">
                  <img src="/icon-128.png" alt="KONI Logo" className="mx-auto size-12" />
                  <h3
                    className={`text-2xl font-semibold tracking-tight ${isDarkMode ? 'text-sky-200' : 'text-koni-navy-900'}`}>
                    {t('welcome_title')}
                  </h3>
              ...
              <>
                {messages.length === 0 && (
                  <div className="flex flex-1 flex-col overflow-hidden">
                    {/* Centered welcome greeting (Claude-style empty state) */}
                    <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
                      {/* High-res KONI emblem, transparent PNG (side-panel/public/koni-hero.png) — no crop needed */}
                      <img src="koni-hero.png" alt="KONI" className="size-44" />
                      <h2
                        className={`text-xl font-semibold tracking-tight ${isDarkMode ? 'text-sky-100' : 'text-koni-navy-900'}`}>
                        {t('chat_greeting')}
                      </h2>
                    </div>
                    {/* Walking KONI mascot, right above the composer */}
                    <KoniMascot />
                    {/* Composer pinned at the bottom */}
                    <div
                      className={`border-t ${isDarkMode ? 'border-sky-900' : 'border-koni-neutral-200'} p-2 shadow-[0_1px_3px_0_rgba(0,0,0,0.06)]`}>
                      <ChatInput
                ...
                {/* Plan approval card — shown while the executor is paused on PLAN_WAIT */}
                {pendingPlan !== null && messages.length > 0 && (
                ...
                {/* ask_user question card — the run ended awaiting the user's typed answer */}
                {pendingQuestion !== null && messages.length > 0 && (
                  <div className="px-3 pb-2">
                    <AskUserCard question={pendingQuestion} isDarkMode={isDarkMode} />
                  </div>
                )}
                {/* Mascot only lives on the fresh empty state — once a task is sent it stays out of the way */}
                {messages.length > 0 && (
                  <div
                    className={`border-t ${isDarkMode ? 'border-sky-900' : 'border-koni-neutral-200'} p-2 shadow-[0_1px_3px_0_rgba(0,0,0,0.06)]`}>
                    <ChatInput
