...
export class NavigatorAgent extends BaseAgent<z.ZodType, NavigatorResult> {
  ...
  async invoke(inputMessages: BaseMessage[]): Promise<this['ModelOutput']> {
    // Ollama first: native constrained decoding (see BaseAgent.invokeOllamaStructured).
    // The dynamic zod schema validates current_state + the action array shape.
    ...
    if (this.withStructuredOutput) {
      // The output schema is hashed ahead of the system prompt in the provider's
      // cache prefix, so a schema that differs by one character throws away the
      // whole prefix behind it. Logged per call: if this number moves, that is
      // the miss, and nothing about the messages needs looking at.
      const sj = JSON.stringify(this.jsonSchema);
      let sh = 0;
      for (let i = 0; i < sj.length; i++) sh = (Math.imul(sh, 31) + sj.charCodeAt(i)) | 0;
      logger.info(`schemafp ${(sh >>> 0).toString(36)}:${sj.length}`);
      ...
      try {
        ...
        // Recover from whatever raw text is reachable (raw content or
        // OutputParserException.llmOutput) — no error-message sniffing; the
        // old 'is not valid JSON' check missed V8's trailing-garbage phrasing
        // ("Unexpected non-whitespace character after JSON", seen with gpt-5.5).
      ...
      // ChatOllama (and others) frequently return the result as a tool call with
      // empty content. Recover it defensively: the state key can be snake_case or
      // camelCase, and `action` must be coerced to an array so a missing or scalar
      // value can't crash the whole step with "action is not iterable".
      if (rawResponse.tool_calls && rawResponse.tool_calls.length > 0) {
        const args = (rawResponse.tool_calls[0]?.args ?? {}) as Record<string, unknown>;
        const currentState = args.current_state ?? args.currentState;
        const rawAction = args.action;
        const action = Array.isArray(rawAction) ? rawAction : rawAction != null ? [rawAction] : [];
        if (currentState !== undefined && action.length > 0) {
          return { current_state: currentState, action: [...action] } as this['ModelOutput'];
        }
        logger.warning('Navigator tool call missing current_state/action', args);
      }

      // Last resort: pull JSON out of the raw text (also strips <think> blocks).
      if (typeof rawResponse.content === 'string') {
        const manual = this.manuallyParseResponse(rawResponse.content);
  ...
  async execute(): Promise<AgentOutput<NavigatorResult>> {
    ...
    try {
      ...
      // Capture the model's running progress summary for session persistence.
      const memory = (modelOutput as { current_state?: { memory?: unknown } })?.current_state?.memory;
      if (typeof memory === 'string' && memory.length > 0) {
        this.context.progressMemory = memory;
      }
      ...
      // A response with no usable action used to burn a whole step (and, in
      // one run, 11 of 30 steps in a row) while the planner kept re-planning.
      // Ask once more with the problem named, the way a parse error is fed
      // back; only if that also comes back empty does the step record the error.
      if (!actions.some(a => a && typeof a === 'object' && Object.keys(a).length > 0)) {
        logger.info('Navigator returned no action; re-asking. raw output:', JSON.stringify(modelOutput).slice(0, 3000));
        const retryMessages = [
          ...inputMessages,
          new HumanMessage(
            'Your previous response contained no action. Reply again with the same JSON shape and a non-empty "action" array (at least one action object such as {"click_element": {...}} or {"done": {...}}).',
          ),
        ];
        const retryOutput = await this.invoke(retryMessages);
        const retryActions = this.fixActions(retryOutput);
        if (retryActions.some(a => a && typeof a === 'object' && Object.keys(a).length > 0)) {
          Object.assign(modelOutput, retryOutput);
  ...
  public async addStateMessageToMemory() {
    ...
    this.flushActionResultsToMemory();
  ...
  /**
   * Flush the pending action results marked includeInMemory into the message
   * history ("Action result:" / "Action error:" lines), resetting each flushed
   * slot so it is never added twice. Normally runs as part of the next step's
   * state build (addStateMessageToMemory); the executor also calls it directly
   * when a run ends on ask_user ('awaiting_user'), because there is no next
   * step and the question must land in the durable transcript before the
   * user's answer arrives.
   */
  public flushActionResultsToMemory() {
    const messageManager = this.context.messageManager;
  ...
  private async doMultiAction(actions: Record<string, unknown>[]): Promise<ActionResult[]> {
    ...
    let errCount = 0;
    // Run logs read this line — the objects themselves print as "[Object]".
    logger.info('Actions', JSON.stringify(actions).slice(0, 1500));
    ...
    for (const [i, action] of actions.entries()) {
      // the model occasionally emits null/empty entries in the action array —
      // Object.keys(null) here would kill the whole step (and, repeated, the task)
      if (!action || typeof action !== 'object' || Object.keys(action).length === 0) {
        results.push(new ActionResult({ error: `Invalid empty action at position ${i}`, includeInMemory: true }));
        continue;
      }
      // One action per array entry: if the model packs several into one object,
      // only the first key runs. Say which ones were dropped — silently losing
      // them leaves the model believing it already did them.
      // A field id ("insured-name") names the same control no matter what moved
      // above it; turn it into this step's index once, so every action below —
      // and page.ts — keeps taking a plain number.
      const ref = (action[Object.keys(action)[0]] as { index?: unknown })?.index;
      if (typeof ref === 'string') {
        const hit = [...browserState.selectorMap.entries()].find(([, el]) => el.attributes?.field === ref);
        if (hit) {
          (action[Object.keys(action)[0]] as { index: number }).index = hit[0];
        } else {
          const known = [...browserState.selectorMap.values()]
            .map(el => el.attributes?.field)
            .filter(Boolean)
            .join(', ');
          results.push(
            new ActionResult({
              error: `No field '${ref}' on this screen. Use an index, or one of: ${known || '(none — this screen has no named fields)'}`,
      ...
      if (actionKeys.length > 1) {
        const msg = `Only '${actionName}' ran. ${actionKeys.slice(1).join(', ')} were in the same object and were dropped — put one action per array entry and send them again.`;
        logger.warning(msg);
        results.push(new ActionResult({ extractedContent: msg, includeInMemory: true }));
      }
      // Expose the executing action's metadata so emitEvent tags its act.* events.
      this.context.currentAction = { actionName, actionIndex: i, actionCount: actions.length };
      try {
        // check if the task is paused or stopped
        if (this.context.paused || this.context.stopped) {
          return results;
        ...
        // done batch termination: weak models often append junk actions after
        // done (observed live: done + go_to_url("") in one batch — the junk
        // action's failure became the LAST result, which is what the done
        // check reads, so a completed task turned into TASK_FAIL). done ends
        // the batch exactly like ask_user below.
        if (result.isDone) {
          const dropped = actions.length - i - 1;
          if (dropped > 0) {
            logger.info(`done executed — dropping ${dropped} remaining action(s) in this batch`);
          }
          return results;
        }

        // ask_user batch termination (Wave 3): once a question is pending,
        // executing the remaining actions of this batch (e.g. a submit queued
        // right after the ask) would defeat the point of asking — drop them.
        if (this.context.askUserPending !== null) {
          const dropped = actions.length - i - 1;
          if (dropped > 0) {
            logger.info(`ask_user pending — dropping ${dropped} remaining action(s) in this batch`);
          }
          return results;
        }
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        // The message is fed back to the model as an action error; it is part of
        // the normal retry loop, not a fault of the extension.
        logger.info(
          'doAction error',
          actionName,
