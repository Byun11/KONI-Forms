...
export function extractJsonFromModelOutput(content: string): Record<string, unknown> {
  try {
    ...
    // Parse the cleaned content. Models may append prose or a second object
    // AFTER a valid JSON object (V8: "Unexpected non-whitespace character
    // after JSON…" — observed live with gpt-5.5's navigator responses), which
    // breaks a whole-string parse; fall back to the first balanced object.
    try {
      return JSON.parse(processedContent);
    } catch (parseError) {
      const first = extractFirstJson(processedContent);
...
/**
 * Extract the FIRST complete JSON object from a mixed text blob, tolerating
 * anything before or after it. Regex/whole-string parsing cannot cut a JSON
 * span with trailing garbage; this walks brace depth while respecting string
 * literals and escapes, and tries each '{' as a start until a balanced span
 * parses to an object.
 */
export function extractFirstJson(text: string): Record<string, unknown> | undefined {
      ...
      if (ch === '"') {
        ...
        if (depth === 0) {
          try {
            ...
            // balanced span didn't parse — advance to the next '{' start
          }
          break;
        }
      }
