...
import { MAX_PROMPT_TOKENS } from '@src/background/agent/types';
...
export default class MessageManager {
  ...
  public initTaskMessages(systemMessage: SystemMessage, task: string, messageContext?: string): void {
    ...
    this.addMessageWithTokens(historyStartMessage, 'init');
  ...
  /**
   * Adds a mid-task user message to the history (Wave 3 mid-run chat / the
   * answer path uses addNewTask instead). Softer than addNewTask: the ultimate
   * task is unchanged — this is extra guidance the user typed while the agent
   * was running, injected at a step boundary.
   * @param text - The raw user message text
   */
  public addUserUpdate(text: string): void {
    const cleaned = filterExternalContent(text);
    const content = `User message (mid-task): """${cleaned}""". Take it into account for the next steps. The ultimate task is unchanged unless this message says otherwise.`;
  ...
  /** Current history size, against which cutMessages() trims. */
  ...
  public getMessages(): BaseMessage[] {
    ...
    // Prompt-cache diagnosis. The provider reuses the longest identical prefix,
    // so what decides the hit rate is the index at which two consecutive calls
    // first differ — a total can never show that. One fingerprint per message
    // makes the divergence point readable straight from the log: line up two
    // msgfp lines and the first column that changes is the culprit.
    logger.info(
      `msgfp ${this.history.messages
        .map(m => {
  ...
  /**
   * Brings the history back under the token budget.
   *
   * Oldest step history goes first; the newest message — the page the agent is
   * about to act on — is only touched once there is no history left to drop.
   * It used to be the other way round, which meant a long run blinded itself:
   * stale action logs survived while the current screen was truncated away.
   */
  public cutMessages(): void {
    ...
    while (diff > 0) {
      const freed = this.history.dropOldestStepMessage();
      if (freed === 0) break;
      diff -= freed;
      logger.debug(
        `Dropped ${freed} tokens of old history - total tokens now: ${this.history.totalTokens}/${this.settings.maxInputTokens}`,
      );
    }
    if (diff <= 0) return;
