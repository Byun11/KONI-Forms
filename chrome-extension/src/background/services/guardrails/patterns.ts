...
export const SECURITY_PATTERNS: SecurityPattern[] = [
  ...
  // Korean task-override attempts. NOTE: \b is useless around Hangul (JS \w
  // is [A-Za-z0-9_], so every Hangul character sits at a "word boundary"),
  // hence no \b anchors here — the patterns stay conservative by requiring an
  // instruction noun plus an override verb / injection marker instead.
  {
    pattern: /(이전|위|기존|모든)\s*(지시|명령|규칙|프롬프트)[^\n]{0,15}?(무시|잊어|잊고|따르지)/g,
    type: ThreatType.TASK_OVERRIDE,
    description: 'Korean attempt to override previous instructions',
    replacement: '[BLOCKED_OVERRIDE_ATTEMPT]',
  },
  {
    pattern: /(새로운?|진짜|실제)\s*(임무|지시|명령|목표)\s*[은는이가:]/g,
    type: ThreatType.TASK_OVERRIDE,
    description: 'Korean attempt to inject a new task',
    replacement: '[BLOCKED_TASK_INJECTION]',
  },
