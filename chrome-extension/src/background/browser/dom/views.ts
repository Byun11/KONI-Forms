...
export class DOMElementNode extends DOMBaseNode {
  ...
  /*
	...
	*/
  isNew: boolean | null;
  /** axtree observation: CDP backend node id — locateElement adopts it directly. */
  backendNodeId?: number;
  /** axtree observation: visible non-interactive text rendered as an unindexed context line. */
  contextText?: string;
  ...
  clickableElementsToString(includeAttributes: string[] | null = null): string {
    ...
    const processNode = (node: DOMBaseNode, depth: number): void => {
      ...
      if (node instanceof DOMElementNode) {
          ...
          if (includeAttributes) {
            ...
            if (Object.keys(attributesToInclude).length > 0) {
              ...
              attributesHtmlStr = Object.entries(attributesToInclude)
                // Name-bearing attrs (the field's human label) get a longer cap so
                // accessible names like "Date of birth (MM/DD/YYYY)" aren't truncated.
                .map(([key, value]) => {
                  // options must survive whole — a half-printed choice list is
                  // worse than none, the model then types the truncated string.
                  // A truncated value reads as a half-filled field: a run spent five
                  // steps retyping an account name because the observation kept
                  // showing "value=Ardent" for "Ardent Packaging Solutions LLC".
                  const LONG: Record<string, number> = { options: 400, field: 60, group: 60, value: 140 };
                  const cap = LONG[key] ?? (key === 'aria-label' || key === 'title' || key === 'placeholder' ? 50 : 15);
                  return `${key}=${capTextLength(value, cap)}`;
                })
                .join(' ');
            }
          }
        ...
        } else if (node.contextText) {
          // axtree context line: visible text that is not a control (label, heading, cell)
          formattedText.push(`${depthStr}<${node.tagName}>${capTextLength(node.contextText, 120)}`);
        }
