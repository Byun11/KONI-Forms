...
/** How many page states to remember when deciding whether an action got anywhere. */
export const FINGERPRINT_HISTORY = 8;

/**
 * Decide what a freshly taken page fingerprint says about the action that
 * produced it, given the ones before it.
 *
 * `unchanged` is the obvious failure: the page is byte-for-byte where it was.
 * `revisited` is the one a before/after comparison misses — a toggle answers
 * every click, so the page really does change each time while cycling between
 * the same two states. Both leave an agent repeating itself until its step
 * budget is gone.
 *
 * The history is mutated in place and trimmed to FINGERPRINT_HISTORY entries.
 */
export function classifyFingerprint(
  ...
  if (!fingerprint) {
    return { unchanged: false, revisited: false };
  }
  const unchanged = history[history.length - 1] === fingerprint;
  // The immediately preceding entry is the 'unchanged' case, not a revisit.
  const revisited = !unchanged && history.slice(0, -1).includes(fingerprint);
  history.push(fingerprint);
  while (history.length > FINGERPRINT_HISTORY) {
    history.shift();
  }
  return { unchanged, revisited };
}
