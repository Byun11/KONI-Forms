window.buildDomTree = (
  ...
  // ---------------------------------------------------------------------------
  // KONI a11y semantic enrichment (clean-room WAI-ARIA accname; not copied code).
  // Only WRITES to nodeData.attributes — never touches highlightIndex/selectorMap.
  // ---------------------------------------------------------------------------
  // KONI_A11Y_HELPERS_START
  const koniClean = s => (s || '').replace(/\s+/g, ' ').trim();
  function computeAccessibleName(el) {
    const aria = el.getAttribute('aria-label');
    if (koniClean(aria)) return koniClean(aria);
    ...
    // Native label association — covers BOTH <label for=id> and a wrapping
    // <label><input></label>, computed by the DOM (no CSS.escape needed).
    ...
    if (koniClean(ph)) return koniClean(ph);
    const title = el.getAttribute('title');
    if (koniClean(title)) return koniClean(title);
    const alt = el.getAttribute('alt');
    if (koniClean(alt)) return koniClean(alt);
    if (el.tagName.toLowerCase() === 'input') {
      const inputType = (el.getAttribute('type') || '').toLowerCase();
      // submit/button/reset: the visible button label lives in the value attribute
      if ((inputType === 'submit' || inputType === 'button' || inputType === 'reset') && koniClean(el.value)) {
        return koniClean(el.value);
      }
      // last resort: the field's own current text (short, never a secret)
      if (!isSensitive(el) && el.value && el.value.length < 50 && koniClean(el.value)) {
        return koniClean(el.value);
      }
    }
    return '';
  }
  // Currently-selected <option> text of a <select> (its visible current value).
  function koniSelectedText(el) {
    const sel = (el.selectedOptions && el.selectedOptions[0]) || (el.options && el.options[el.selectedIndex]);
    return sel ? koniClean(sel.textContent) : '';
  }
  // Sensitive-field test (clean-room; from the public HTML autocomplete token list).
  const KONI_SENSITIVE_AC = [
    'current-password',
    'new-password',
    'one-time-code',
    'cc-number',
  ...
  function isSensitive(el) {
    ...
    return KONI_SENSITIVE_AC.some(tok => ac.includes(tok));
  }
  // Implicit ARIA role (clean-room subset of the public HTML-AAM mappings).
  const KONI_INPUT_ROLES = {
    submit: 'button',
    button: 'button',
    reset: 'button',
    image: 'button',
  ...
  const KONI_TAG_ROLES = { a: 'link', button: 'button', select: 'combobox', textarea: 'textbox' };
  function computeRole(el) {
    ...
    if (tag === 'input') {
      ...
      return KONI_INPUT_ROLES[t] || 'textbox';
    }
    return KONI_TAG_ROLES[tag] || '';
  }
  // KONI_A11Y_HELPERS_END
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false, depth = 0) {
    ...
    if (
      ...
      // KONI Item A: fold the computed accessible name (incl. <label for>) into
      // aria-label for form fields, only when the author didn't supply one.
      const koniTag = node.tagName.toLowerCase();
      if (
        (koniTag === 'input' || koniTag === 'select' || koniTag === 'textarea') &&
        !nodeData.attributes['aria-label']
      ) {
        const accName = computeAccessibleName(node);
        if (accName) nodeData.attributes['aria-label'] = accName.slice(0, 100);
      }
      // KONI Item C: implicit ARIA role so the model chooses the right action
      // (textbox -> input_text, combobox/select -> select_dropdown_option, etc.).
      if ((koniTag === 'input' || koniTag === 'select' || koniTag === 'textarea') && !nodeData.attributes['role']) {
        const role = computeRole(node);
        if (role) nodeData.attributes['role'] = role;
      }
      // KONI: surface a <select>'s current selection (as value) so the model sees
      // what's chosen; the label already gives the field name. Redacted below if secret.
      if (koniTag === 'select' && !nodeData.attributes.value) {
        const chosen = koniSelectedText(node);
        if (chosen) nodeData.attributes.value = chosen.slice(0, 50);
      }
      // KONI Item B: redact secret values (password/OTP/card) so they never reach
      // the model or chat history. The agent can still FILL the field by index.
      if (isSensitive(node) && nodeData.attributes.value) {
        nodeData.attributes.value = '[redacted]';
      }
    }
